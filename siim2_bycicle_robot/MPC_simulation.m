...
%     disp(U_opt(:,1:10))
    ...
    h = figure(1);  
    h = figure(1);  
    h.Position = [0 500 400 430]; % Adjust dimensions as needed
...
%     set(gcf,'Position',[state(1) state(2) 50 50],'Resize','off');
    ...
    set(gca, 'Position', [0.03, -0.05, 0.95, 0.99]);
    box on;
    


    title("MPPI for Robot Trajectory Tracking", 'FontSize', 10, 'interpreter','latex');
    subtitle("with Obstacles", 'FontSize', 10, 'interpreter','latex')
    
   
    
    saveas(gcf, "imgs/Robot_with_obstacles.png")
    print -depsc 'imgs/Robot_with_obstacles.eps'
